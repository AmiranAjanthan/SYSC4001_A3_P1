 ...
 * @author Amiran Ajanthan
 * @author Fareen Lavji
 * @brief template main.cpp file for Assignment 3 Part 1 of SYSC4001
 * 
 */

#include"interrupts_101303353_101036543.hpp"
...
std::tuple<std::string /* add std::string for bonus mark */ > run_simulation(std::vector<PCB> list_processes) {
    ...
    unsigned int cpu_execution_time = 0;
    const unsigned int time_slice = 100;
    unsigned int time_slice_remaining = time_slice;
    ...
    while(!all_process_terminated(job_list) || job_list.empty()) {
        ...
        if (!ready_queue.empty()){
            ...
            if (running.state == NOT_ASSIGNED || (ready_queue.back().PID < running.PID && !ready_queue.empty())){
        ...
        if (running.state == RUNNING){
            ...
            if (running.io_freq > 0 && cpu_execution_time == running.io_freq && running.remaining_time > 0){
            ...
            else if (time_slice_remaining == 0 && running.remaining_time > 0){
                running.state = READY;
                ready_queue.push_back(running);
